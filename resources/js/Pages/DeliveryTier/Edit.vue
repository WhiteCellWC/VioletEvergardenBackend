...
const form = useForm({
    _method: 'PUT',
    delivery_option_id: props.deliveryTier.data.delivery_option_id,
    max_weight: props.deliveryTier.data.max_weight,
    price: props.deliveryTier.data.price,
    status: props.deliveryTier.data.status
})
const deliveryOptionLabel = ref(props.deliveryTier.data.delivery_option.name);

// Handlers
const submit = () => {
    form.post(route('delivery-tiers.update', { delivery_tier: props.deliveryTier.data.id }))
}
        ...
        <template #content>
            ...
            <form @submit.prevent="submit" class="p-8 flex flex-col w-1/2 gap-6">
                <div class="flex flex-col gap-2">
                    ...
                    <Dropdown v-model="form.delivery_option_id" v-model:modelLabel="deliveryOptionLabel"
                        :fetchRoute="route('api.v1.delivery-options.index')" :valueKey="'id'" :labelKey="'name'"
                        placeholder="Select Delivery Option" :allowSearch="true" />
                </div>
